(function() {
    ...
    const RSS_URL = `https://${hostname}/s/search.json?collection=stir-www&query=!padrenullquery&meta_d3year=2017&profile=_default_preview`
    ...
    const getPages = (tag,ranks) => {
        const fb = `https://${hostname}/s/search.json?collection=stir-www&profile=_default_preview&meta_tag=${tag}&meta_d3year=2017&num_ranks=${ranks||255}&SF=[c,d,tag]&sort=date`;
        fetch(fb)
