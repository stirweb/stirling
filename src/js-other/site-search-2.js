...
(function () {
  ...
  var intLabel = document.getElementById("c-site-search-internal__label");
  ...
  var onLoad = function () {
    ...
    if (getAuthUserType() === "STUDENT")
      if (intLabel) intLabel.innerHTML = "Student pages";
    ...
    if (extFilters) {
      if (getAuthUserType() === "STAFF" || getAuthUserType() === "STUDENT") {
        ...
        if (extCheckbox.checked && intCheckbox.checked) {
          if (getAuthUserType() === "STUDENT")
            searchFacets.meta_subgroup_not = "staff";
        }
  ...
  var doSearch = function () {
    ...
    for (var key in searchFacets)
      if (searchFacets.hasOwnProperty(key))
        jsonUrlFull += key + "=" + searchFacets[key] + "&";
    ...
    if (!data.error) {
      ...
      resultSummaryArea
        ? (resultSummaryArea.innerHTML =
            StirSearchHelpers.formSearchSummaryHTML(
              totalPosts,
              searchFacets.start_rank,
              lastPost
            ))
        : "";
      ...
      if (pagination) {
        ...
        if (postsPerPage < totalPosts)
          pagination.innerHTML = StirSearchHelpers.formPaginationHTML(
            totalPosts,
            postsPerPage,
            page,
            noOfPageLinks
   ...
   *  Function: Deep links
   * ------------------------------------------ */
  var updateDeepLinkUrls = function () {
  ...
  function renderIcon(iconType) {
    ...
    return (
      '<span class="c-site-search__external-icon-wrapper"><span class="c-site-search__external-icon ' +
      icon +
      ...
      for (var key in curs) {
        ...
        if (val.titleHtml) {
          ...
          if (searchFacets.meta_group == "internal") {
            ...
            if (
              getSiteArea(val.displayUrl) === "STUDENT" &&
              getAuthUserType() !== "EXTERNAL"
            ) {
              html.push('<div class="cell small-12 c-search-result">');
              html.push(
                '<p class="c-search-result__link"><a href="' +
                  val.linkUrl +
                  '">' +
                  val.titleHtml +
                  "</a> " +
              ...
              html.push(
                '<p class="c-search-result__summary">' +
                  val.descriptionHtml +
                  "</p>"
              );
              html.push("</div>");
            ...
            // Internal Staff Only
            if (
              getSiteArea(val.displayUrl) === "STAFF" &&
              getAuthUserType() === "STAFF"
            ) {
              html.push('<div class="cell small-12 c-search-result">');
              html.push(
                '<p class="c-search-result__link"><a href="' +
      ...
      if (val2.displayUrl) {
        ...
        html.push(
          '<p class="c-search-result__link"><a href="' +
            url +
            '">' +
            stir.String.getFirstFromSplit.call(val2.title, "|") +
            "</a>" +
            iconHtml +
            "</p>"
        );
        html.push(
          '			<p class="c-search-result__breadcrumb">' +
            (val2.metaData.bc || val2.displayUrl) +
            "</p>"
        );
        html.push(
          '<p class="c-search-result__summary">' + val2.summary + "</p>"
        );
        html.push("</div>");
      }
    }
