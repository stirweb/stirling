(function () {
	...
	const getFacetsFromMetaTags = (name) => {
		const selector = `meta[name="addsearch-custom-field"][content^="${name}"]`;
		return Array.prototype.slice.call(document.querySelectorAll(selector))
			.map((el) => el.content
			.replace(`${name}=`,''))
			.filter(i=>i!=="All Faculties");
	};
	...
	document.querySelectorAll("[data-funnelback-inject],[data-search-inject]").forEach((el) => {
		...
		// Don't include the current page in the results
		if (sid) {
			const filter = {
				and: [{ not: { "custom_fields.sid": sid } }],
			};
			params.append("filter", JSON.stringify(filter));
		}
	
		level &&
		levels[level] && params.append("customField", `level=${levels[level]}`);
