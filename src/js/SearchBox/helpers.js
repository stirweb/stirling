...
var SearchBoxViewHelpers = (function() {
    ...
    var _generateSuggestUrlBySubgroup = function(base_url, subgroup) {
        ...
        switch (subgroup) {
            case "staff":
                params = $.extend({ // search all (use checkboxes to filter)
                    // ...
                }, params);
                break;
            case "students":
                params = $.extend({  // only include students (no staff or external)
                    meta_subgroup_and: "student"
                }, params);
                break;
            default: // e.g. external
                params = $.extend({  // don't include internal (staff or students)
    ...
    var _addFiltersFromQueryString = function(searchBox) {
        var params = QueryParams.getAll(), filterName, filterValues;
        for (var name in params) {
            if (name.match(/^filter__/)) {
                ...
                for (var i=0; i<filterValues.length; i++) {
                    ...
                    if (filterValues[i]) { // don't set if empty
