!function(t){if(t){t&&(t.value=window.location.href);t=stir.node("#copyurl");t&&t.addEventListener("click",t=>(async()=>{await navigator.clipboard.writeText(window.location.href)})())}}(stir.node("#shareurl")),function(){const o=stir.node("#seriesevents"),r=o&&o.dataset&&o.dataset.seriesid?o.dataset.seriesid:null,a=stir.node("#seriesdatefilter"),l=stir.node("#moreevents");if(!o&&!l)return;const e=t=>`<span class="u-bg-heritage-berry u-white c-tag u-mr-1 u-inline-block u-mb-1">${t}</span><br/>`;const s=t=>{t=new Date(t);return t.getDate()+" "+["January","February","March","April","May","June","July","August","September","October","November","December"][t.getMonth()]+" "+t.getFullYear()},u=t=>`<option value="${t}">${s(t)}</option>`;const c=stir.map(t=>{return`<a href="${t.url}" class="u-border u-p-1 u-mb-1 flex-container flex-dir-column large-flex-dir-row   u-gap">
            ...
            </a>`}),p=stir.filter((t,r)=>r<3),i=stir.curry((t,r)=>{var e;return""===t||(e=C(r.start,r.end),D(e,t))?r:void 0}),g=stir.curry((r,e)=>{var s={},i=[];for(let t=0;t<e.length;t++)s[e[t][r]]||(s[e[t][r]]=!0,i.push(e[t]));return i}),n=t=>t.start,v=stir.map((t,r)=>{return`
      <div class="u-bg-white ${0===r?"u-heritage-line-top u-border-width-5":"u-grey-line-top "} u-p-1 c-event-list u-gap">
        <div>
      ...
      </div>`}),m=stir.join(""),h=()=>{var t=new Date;return Number(t.toISOString().split("T")[0].split("-").join("")+("0"+t.getHours()).slice(-2)+("0"+t.getMinutes()).slice(-2))},f=(t,r)=>t.startInt-r.startInt;const w=stir.filter(t=>t.endInt>=h());const b=stir.filter(t=>Number(t.endInt)<h()&&t.archive.length),$=stir.filter(t=>t.isSeriesChild===r),y=(t,r)=>Number(t.pin)-Number(r.pin);const x=stir.curry((t,r)=>(stir.setHTML(t,r),!0)),k=stir.curry((t,r)=>{t=t.split(", ");if(!t&&!t.length)return r;if(1===t.length&&""===t[0])return r;const e=r.tags.split(", ");t=t.map(t=>e.includes(t));return stir.any(t=>t,t)?r:void 0}),D=stir.curry((t,r)=>{return!!t.filter(t=>t===r).length}),C=(t,r)=>{var e=[];for(d=new Date(t);d<=new Date(r);d.setDate(d.getDate()+1))e.push(new Date(d).toISOString().split("T")[0]);return e},S=t=>{var r=new Date;return r.setHours(0,0,0,0),new Date(t)>=r},I=t=>C(t.start,t.end),N=t=>stir.compose(b,stir.sort(f),$,stir.filter(n))(t),F=t=>stir.compose(w,stir.sort(f),$,stir.filter(n))(t),j=(t,r)=>{t=stir.filter(i(t)),r=N(r),t=stir.compose(m,v,stir.sort(f),t)(r);return t.length?""+t:""},M=(t,r)=>{t=stir.filter(i(t)),r=F(r),t=stir.compose(m,v,stir.sort(f),t)(r);return t.length?""+t:""};var t="dev"===(t=UoS_env.name)?"../index.json":"preview"===t||"appdev-preview"===t?'<t4 type="navigation" id="5214" />':"/data/events/revamp/json/index.json";stir.getJSON(t,e=>{if(!e.error){var t=stir.node("#serieseventspast"),r=j("",e);if(t&&(r=""===r?"No passed events found":r,x(t,r)),o&&a&&(x(o,M("",e)),x(a,'<option value="">Filter by upcoming date</option>'+(t=>{var r=N(t),t=F(t),r=([...r,...t],stir.filter(S));stir.compose(stir.removeDuplicates,stir.flatten,stir.map(I))(t);return stir.compose(m,stir.map(u),r,stir.removeDuplicates,stir.flatten,stir.map(I))(t)})(e)),a.addEventListener("change",t=>{var r=M(a.options[a.selectedIndex].value,e),r=(j(a.options[a.selectedIndex].value,e),""===r?"<p><strong>No events on date selected</strong></p>":r);x(o,r)})),l){t=e;r=g("id");const n=l.dataset.currentid||null;var s=n?stir.filter(t=>t.id===Number(n),t):null,s=s.length?s[0]:null,s=s?s.tags:null,s=stir.filter(k(s)),i=stir.filter(t=>t.id!==Number(n)),s=stir.compose(i,s,stir.sort(f),w)(t),i=stir.compose(i,stir.sort(y),stir.sort(f),w)(t),t=[...s,...i];stir.compose(x(l),m,c,p,r)(t)}}})}(),function(t){t&&gallery&&galleryId&&stir.curry((t,r)=>(stir.setHTML(t,r),!0))(t,gallery.map(t=>{return`<img alt="${t.title.length?t.title:"Flickr image "+t.id}" class="u-object-cover"  src="https://farm${t.farm}.staticflickr.com/${t.server}/${t.id}_${t.secret}_c.jpg" width="${t.o_width}" height="${t.o_height}"></img>`}).join("")+`<div>
