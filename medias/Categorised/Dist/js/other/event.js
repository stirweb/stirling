!function(){var t=document.head.querySelector("[name=sid][content]");if(t)for(var e=document.querySelectorAll('a[data-section-id="'+t.getAttribute("content")+'"][data-type="event"]'),r=0;r<e.length;r++)e[r].parentNode.removeChild(e[r])}(),function(t){if(t){t&&(t.value=window.location.href);t=stir.node("#copyurl");t&&t.addEventListener("click",t=>(async()=>{await navigator.clipboard.writeText(window.location.href)})())}}(stir.node("#shareurl")),function(t){if(t){const s=t,e=s.dataset&&s.dataset.seriesid?s.dataset.seriesid:null,n=stir.node("#seriesdatefilter"),a=(t,e)=>t?`<h3 class="header-stripped ${e}">${t}</h3>`:"",r=t=>`<span class="u-bg-heritage-berry u-white c-tag u-mr-1 u-inline-block u-mb-1">${t}</span>`;const o=t=>`<option value="${t.startInt}">${t.stirStart}</option>`,i=stir.curry((t,e)=>""===t||e.startInt===Number(t)?e:void 0),l=t=>({start:t.start,stirStart:t.stirStart,startInt:t.startInt}),d=stir.curry((e,r)=>{var i={},s=[];for(let t=0;t<r.length;t++)i[r[t][e]]||(i[r[t][e]]=!0,s.push(r[t]));return s}),c=t=>t.start,u=stir.map((t,e)=>{return`
        ...
        </div>`}),g=stir.join(""),p=()=>{var t=new Date;return Number(t.toISOString().split("T")[0].split("-").join("")+("0"+t.getHours()).slice(-2)+("0"+t.getMinutes()).slice(-2))},h=(t,e)=>t.startInt-e.startInt;const v=stir.filter(t=>t.endInt>=p());const m=stir.filter(t=>Number(t.endInt)<p()&&t.archive.length),b=stir.filter(t=>t.isSeriesChild===e);const f=stir.curry((t,e)=>(stir.setHTML(t,e),!0)),w=(t,e)=>{t=stir.filter(i(t)),t=stir.compose(g,u,stir.sort(h),t,v,b)(e);return t.length?a("Upcoming","")+t:""};t="dev"===(t=UoS_env.name)?"index.json":"preview"===t?'<t4 type="navigation" id="5214" />':"/data/events/revamp/json/index.json";stir.getJSON(t,r=>{console.log(r);const i=(t=>{t=stir.compose(g,u,stir.sort(h),m,b)(t);return t.length?a("Passed","u-mt-2")+t:""})(r);var t,e;f(s,w("",r)+i),n&&(f(n,'<option value="">Filter by date</option>'+(t=r,e=d("startInt"),stir.compose(g,stir.map(o),e,stir.map(l),stir.sort(h),v,b,stir.filter(c))(t))),n.addEventListener("change",t=>{var e=w(n.options[n.selectedIndex].value,r);f(s,e+i)}))})}}(stir.node("#seriesevents")),function(t){t&&gallery&&galleryId&&stir.curry((t,e)=>(stir.setHTML(t,e),!0))(t,gallery.map(t=>`<img alt="Stirling AMAM Golf Day 2019" class="u-object-cover"  src="https://farm${t.farm}.staticflickr.com/${t.server}/${t.id}_${t.secret}_c.jpg" width="${t.o_width}" height="${t.o_height}"></img>`).join("")+`<div>
