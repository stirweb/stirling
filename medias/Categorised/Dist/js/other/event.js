!function(t){if(t){t&&(t.value=window.location.href);t=stir.node("#copyurl");t&&t.addEventListener("click",t=>(async()=>{await navigator.clipboard.writeText(window.location.href)})())}}(stir.node("#shareurl")),function(){const a=stir.node("#seriesevents"),r=a&&a.dataset&&a.dataset.seriesid?a.dataset.seriesid:null,l=stir.node("#seriesdatefilter"),o=stir.node("#moreevents");if(!a&&!o)return;const e=(t,r)=>t?`<h3 class="header-stripped ${r}">${t}</h3>`:"",s=t=>`<span class="u-bg-heritage-berry u-white c-tag u-mr-1 u-inline-block u-mb-1">${t}</span><br/>`;const i=t=>{t=new Date(t);return t.getDate()+" "+["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t.getMonth()]+" "+t.getFullYear()},c=t=>`<option value="${t}">${i(t)}</option>`;const u=stir.map(t=>{return`<a href="${t.url}" class="u-border u-p-1 u-mb-1 flex-container flex-dir-column large-flex-dir-row   u-gap">
            ...
            </a>`}),g=stir.filter((t,r)=>r<3),p=stir.filter((t,r)=>r<1),n=stir.curry((t,r)=>{var e;return""===t||(e=F(r.start,r.end),S(e,t))?r:void 0}),h=stir.curry((r,e)=>{var s={},i=[];for(let t=0;t<e.length;t++)s[e[t][r]]||(s[e[t][r]]=!0,i.push(e[t]));return i}),v=t=>t.start,m=stir.map((t,r)=>{return`
        ...
        </div>`}),f=stir.join(""),w=()=>{var t=new Date;return Number(t.toISOString().split("T")[0].split("-").join("")+("0"+t.getHours()).slice(-2)+("0"+t.getMinutes()).slice(-2))},b=(t,r)=>t.startInt-r.startInt,$=(t,r)=>r.endInt-t.endInt;const y=stir.filter(t=>t.endInt>=w());const x=stir.filter(t=>Number(t.endInt)<w()&&t.archive.length),k=stir.filter(t=>t.isSeriesChild===r),D=(t,r)=>Number(t.pin)-Number(r.pin);const I=stir.curry((t,r)=>(stir.setHTML(t,r),!0)),C=stir.curry((t,r)=>{t=t.split(", ");if(!t&&!t.length)return r;if(1===t.length&&""===t[0])return r;const e=r.tags.split(", ");t=t.map(t=>e.includes(t));return stir.any(t=>t,t)?r:void 0}),S=stir.curry((t,r)=>{return!!t.filter(t=>t===r).length}),F=(t,r)=>{var e=[];for(d=new Date(t);d<=new Date(r);d.setDate(d.getDate()+1))e.push(new Date(d).toISOString().split("T")[0]);return e},N=(t,r)=>{t=stir.filter(n(t)),t=stir.compose(f,m,stir.sort(b),t,y,k)(r);return t.length?e("Upcoming","")+t:""},j=(t,r)=>{t=stir.filter(n(t)),t=stir.compose(f,m,stir.sort(b),t,x,k)(r);return t.length?e("Passed","u-mt-2")+t:""};var t="dev"===(t=UoS_env.name)?"../index.json":"preview"===t?'<t4 type="navigation" id="5214" />':"/data/events/revamp/json/index.json";stir.getJSON(t,s=>{if(!s.error&&(a&&l&&(t=j("",s),I(a,N("",s)+t),I(l,'<option value="">Filter by date</option>'+(t=s,r=stir.compose(p,stir.sort(b),k,stir.filter(v))(t),t=stir.compose(p,stir.sort($),k,stir.filter(v))(t),r=F(r[0].start,t[0].end),stir.compose(f,stir.map(c))(r))),l.addEventListener("change",t=>{var r=N(l.options[l.selectedIndex].value,s),e=j(l.options[l.selectedIndex].value,s),e=r+e===""?"<p><strong>No events on date selected</strong></p>":r+e;I(a,r+e)})),o)){var t=s;var r=h("id");const n=o.dataset.currentid||null;var e=n?stir.filter(t=>t.id===Number(n),t):null,e=e.length?e[0]:null,e=e?e.tags:null,e=stir.filter(C(e)),i=stir.filter(t=>t.id!==Number(n)),e=stir.compose(i,e,stir.sort(b),y)(t),i=stir.compose(i,stir.sort(D),stir.sort(b),y)(t),t=[...e,...i];stir.compose(I(o),f,u,g,r)(t)}})}(),function(t){t&&gallery&&galleryId&&stir.curry((t,r)=>(stir.setHTML(t,r),!0))(t,gallery.map(t=>{return`<img alt="${t.title.length?t.title:"Flickr image "+t.id}" class="u-object-cover"  src="https://farm${t.farm}.staticflickr.com/${t.server}/${t.id}_${t.secret}_c.jpg" width="${t.o_width}" height="${t.o_height}"></img>`}).join("")+`<div>
