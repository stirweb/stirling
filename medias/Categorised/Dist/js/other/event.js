!function(t){if(t){t&&(t.value=window.location.href);t=stir.node("#copyurl");t&&t.addEventListener("click",t=>(async()=>{await navigator.clipboard.writeText(window.location.href)})())}}(stir.node("#shareurl")),function(){const a=stir.node("#seriesevents"),r=a&&a.dataset&&a.dataset.seriesid?a.dataset.seriesid:null,o=stir.node("#seriesdatefilter"),l=stir.node("#moreevents");if(a||l){const e=(t,r)=>t?`<h3 class="header-stripped ${r}">${t}</h3>`:"",s=t=>`<span class="u-bg-heritage-berry u-white c-tag u-mr-1 u-inline-block u-mb-1">${t}</span>`;const d=t=>`<option value="${t.start}">${t.stirStart}</option>`;const c=stir.map(t=>{return`<a href="#" class="u-border u-p-1 u-mb-1 flex-container flex-dir-column large-flex-dir-row   u-gap">
            ...
            </a>`}),u=stir.filter((t,r)=>r<3),i=stir.curry((t,r)=>(console.log(r.start+" -- "+t),""===t||r.start===t?r:void 0)),g=t=>({start:t.start,stirStart:t.stirStart,startInt:t.startInt}),p=stir.curry((r,e)=>{var s={},i=[];for(let t=0;t<e.length;t++)s[e[t][r]]||(s[e[t][r]]=!0,i.push(e[t]));return i}),h=t=>t.start,n=stir.map((t,r)=>{return`
        ...
        </div>`}),m=stir.join(""),v=()=>{var t=new Date;return Number(t.toISOString().split("T")[0].split("-").join("")+("0"+t.getHours()).slice(-2)+("0"+t.getMinutes()).slice(-2))},f=(t,r)=>t.startInt-r.startInt;const w=stir.filter(t=>t.endInt>=v());const b=stir.filter(t=>Number(t.endInt)<v()&&t.archive.length),$=stir.filter(t=>t.isSeriesChild===r),x=(t,r)=>Number(t.pin)-Number(r.pin);const y=stir.curry((t,r)=>(stir.setHTML(t,r),!0)),k=stir.curry((t,r)=>{t=t.split(", ");if(!t&&!t.length)return r;if(1===t.length&&""===t[0])return r;const e=r.tags.split(", ");t=t.map(t=>e.includes(t));return stir.any(t=>t,t)?r:void 0}),C=(t,r)=>{t=stir.filter(i(t)),t=stir.compose(m,n,stir.sort(f),t,w,$)(r);return t.length?e("Upcoming","")+t:""},I=(t,r)=>{t=stir.filter(i(t)),t=stir.compose(m,n,stir.sort(f),t,b,$)(r);return t.length?e("Passed","u-mt-2")+t:""};var t="dev"===(t=UoS_env.name)?"index.json":"preview"===t?'<t4 type="navigation" id="5214" />':"/data/events/revamp/json/index.json";stir.getJSON(t,s=>{if(!s.error&&(a&&o&&(r=I("",s),y(a,C("",s)+r),y(o,'<option value="">Filter by date</option>'+(r=s,t=p("start"),console.log(stir.compose(t,stir.map(g),stir.sort(f),$,stir.filter(h))(r)),stir.compose(m,stir.map(d),t,stir.map(g),stir.sort(f),$,stir.filter(h))(r))),o.addEventListener("change",t=>{var r=C(o.options[o.selectedIndex].value,s),e=I(o.options[o.selectedIndex].value,s);y(a,r+e)})),l)){var t=s;var r=p("id");const n=l.dataset.currentid||null;var e=n?stir.filter(t=>t.id===Number(n),t):null,e=e.length?e[0]:null,e=e?e.tags:null,e=stir.filter(k(e)),i=stir.filter(t=>t.id!==Number(n)),e=stir.compose(i,e,stir.sort(f),w)(t),i=stir.compose(i,stir.sort(x),stir.sort(f),w)(t),t=[...e,...i];stir.compose(y(l),m,c,u,r)(t)}})}}(),function(t){t&&gallery&&galleryId&&stir.curry((t,r)=>(stir.setHTML(t,r),!0))(t,gallery.map(t=>`<img alt="Stirling AMAM Golf Day 2019" class="u-object-cover"  src="https://farm${t.farm}.staticflickr.com/${t.server}/${t.id}_${t.secret}_c.jpg" width="${t.o_width}" height="${t.o_height}"></img>`).join("")+`<div>
