!function(t){if(t){t&&(t.value=window.location.href);t=stir.node("#copyurl");t&&t.addEventListener("click",t=>(async()=>{await navigator.clipboard.writeText(window.location.href)})())}}(stir.node("#shareurl")),function(){const a=stir.node("#seriesevents"),r=a&&a.dataset&&a.dataset.seriesid?a.dataset.seriesid:null,o=stir.node("#seriesdatefilter"),l=stir.node("#moreevents");if(!a&&!l)return;const e=(t,r)=>t?`<h3 class="header-stripped ${r}">${t}</h3>`:"",i=t=>`<span class="u-bg-heritage-berry u-white c-tag u-mr-1 u-inline-block u-mb-1">${t}</span><br/>`;const s=t=>{t=new Date(t);return t.getDate()+" "+["January","February","March","April","May","June","July","August","September","October","November","December"][t.getMonth()]+" "+t.getFullYear()},c=t=>`<option value="${t}">${s(t)}</option>`;const u=stir.map(t=>{return`<a href="${t.url}" class="u-border u-p-1 u-mb-1 flex-container flex-dir-column large-flex-dir-row   u-gap">
            ...
            </a>`}),g=stir.filter((t,r)=>r<3),n=stir.curry((t,r)=>{var e;return""===t||(e=S(r.start,r.end),C(e,t))?r:void 0}),p=stir.curry((r,e)=>{var i={},s=[];for(let t=0;t<e.length;t++)i[e[t][r]]||(i[e[t][r]]=!0,s.push(e[t]));return s}),v=t=>t.start,h=stir.map((t,r)=>{return`
        <div class="${r%2==1?"":"u-bg-grey"} ${0===r?"u-heritage-line-top u-border-width-5":""} u-p-1 c-event-list u-gap">
          <div >
            ${t.cancelled?i("Cancelled"):""}${t.rescheduled?i("Rescheduled"):""} 
            <span class="u-inline-block u-mb-1"><strong>Event</strong><br />
           ${r=t,r.url?`<a href="${r.url}">${r.title}</a><br />`:r.title+"<br />"}
        ...
        </div>`}),m=stir.join(""),f=()=>{var t=new Date;return Number(t.toISOString().split("T")[0].split("-").join("")+("0"+t.getHours()).slice(-2)+("0"+t.getMinutes()).slice(-2))},b=(t,r)=>t.startInt-r.startInt;const w=stir.filter(t=>t.endInt>=f());const $=stir.filter(t=>Number(t.endInt)<f()&&t.archive.length),y=stir.filter(t=>t.isSeriesChild===r),x=(t,r)=>Number(t.pin)-Number(r.pin);const k=stir.curry((t,r)=>(stir.setHTML(t,r),!0)),D=stir.curry((t,r)=>{t=t.split(", ");if(!t&&!t.length)return r;if(1===t.length&&""===t[0])return r;const e=r.tags.split(", ");t=t.map(t=>e.includes(t));return stir.any(t=>t,t)?r:void 0}),C=stir.curry((t,r)=>{return!!t.filter(t=>t===r).length}),S=(t,r)=>{var e=[];for(d=new Date(t);d<=new Date(r);d.setDate(d.getDate()+1))e.push(new Date(d).toISOString().split("T")[0]);return e},I=t=>S(t.start,t.end),F=t=>stir.compose($,stir.sort(b),y,stir.filter(v))(t),N=t=>stir.compose(w,stir.sort(b),y,stir.filter(v))(t),j=(t,r)=>{t=stir.filter(n(t)),r=F(r),t=stir.compose(m,h,stir.sort(b),t)(r);return t.length?e("Passed","u-mt-2")+t:""},M=(t,r)=>{t=stir.filter(n(t)),r=N(r),t=stir.compose(m,h,stir.sort(b),t)(r);return t.length?e("Upcoming","")+t:""};var t="dev"===(t=UoS_env.name)?"../index.json":"preview"===t||"appdev-preview"===t?'<t4 type="navigation" id="5214" />':"/data/events/revamp/json/index.json";stir.getJSON(t,i=>{if(!i.error&&(a&&o&&(t=j("",i),k(a,M("",i)+t),k(o,'<option value="">Filter by upcoming date</option>'+(t=>{var r=F(t),t=N(t);[...r,...t];return stir.compose(m,stir.map(c),stir.removeDuplicates,stir.flatten,stir.map(I))(t)})(i)),o.addEventListener("change",t=>{var r=M(o.options[o.selectedIndex].value,i),e=j(o.options[o.selectedIndex].value,i),r=r+e===""?"<p><strong>No events on date selected</strong></p>":r+e;k(a,r)})),l)){var t=i;var r=p("id");const n=l.dataset.currentid||null;var e=n?stir.filter(t=>t.id===Number(n),t):null,e=e.length?e[0]:null,e=e?e.tags:null,e=stir.filter(D(e)),s=stir.filter(t=>t.id!==Number(n)),e=stir.compose(s,e,stir.sort(b),w)(t),s=stir.compose(s,stir.sort(x),stir.sort(b),w)(t),t=[...e,...s];stir.compose(k(l),m,u,g,r)(t)}})}(),function(t){t&&gallery&&galleryId&&stir.curry((t,r)=>(stir.setHTML(t,r),!0))(t,gallery.map(t=>{return`<img alt="${t.title.length?t.title:"Flickr image "+t.id}" class="u-object-cover"  src="https://farm${t.farm}.staticflickr.com/${t.server}/${t.id}_${t.secret}_c.jpg" width="${t.o_width}" height="${t.o_height}"></img>`}).join("")+`<div>
