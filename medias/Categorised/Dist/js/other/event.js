!function(t){if(t){t&&(t.value=window.location.href);t=stir.node("#copyurl");t&&t.addEventListener("click",t=>(async()=>{await navigator.clipboard.writeText(window.location.href)})())}}(stir.node("#shareurl")),function(){const o=stir.node("#seriesevents"),r=o&&o.dataset&&o.dataset.seriesid?o.dataset.seriesid:null,l=stir.node("#seriesdatefilter"),d=stir.node("#moreevents");if(o||d){const c=(t,r)=>t?`<h3 class="header-stripped ${r}">${t}</h3>`:"",e=t=>`<span class="u-bg-heritage-berry u-white c-tag u-mr-1 u-inline-block u-mb-1">${t}</span>`;const u=t=>`<option value="${t.startInt}">${t.stirStart}</option>`;const g=stir.map(t=>{return`<a href="#" class="u-border u-p-1 u-mb-1 flex-container flex-dir-column large-flex-dir-row   u-gap">
        ...
        </div>`}),w=stir.join(""),i=()=>{var t=new Date;return Number(t.toISOString().split("T")[0].split("-").join("")+("0"+t.getHours()).slice(-2)+("0"+t.getMinutes()).slice(-2))},b=(t,r)=>t.startInt-r.startInt;const $=stir.filter(t=>t.endInt>=i());const y=stir.filter(t=>Number(t.endInt)<i()&&t.archive.length),x=stir.filter(t=>t.isSeriesChild===r),k=(t,r)=>Number(t.pin)-Number(r.pin);const I=stir.curry((t,r)=>(stir.setHTML(t,r),!0)),C=stir.curry((t,r)=>{t=t.split(", ");if(!t&&!t.length)return r;if(1===t.length&&""===t[0])return r;const e=r.tags.split(", ");t=t.map(t=>e.includes(t));return stir.any(t=>t,t)?r:void 0}),S=(t,r)=>{t=stir.filter(s(t)),t=stir.compose(w,f,stir.sort(b),t,$,x)(r);return t.length?c("Upcoming","")+t:""};var t="dev"===(t=UoS_env.name)?"index.json":"preview"===t?'<t4 type="navigation" id="5214" />':"/data/events/revamp/json/index.json";stir.getJSON(t,e=>{if(!e.error){if(o&&l){const n=(t=>{t=stir.compose(w,f,stir.sort(b),y,x)(t);return t.length?c("Passed","u-mt-2")+t:""})(e);I(o,S("",e)+n),I(l,'<option value="">Filter by date</option>'+(r=e,t=m("startInt"),stir.compose(w,stir.map(u),t,stir.map(h),stir.sort(b),$,x,stir.filter(v))(r))),l.addEventListener("change",t=>{var r=S(l.options[l.selectedIndex].value,e);I(o,r+n)})}if(d){var t=e;var r=m("id");const a=d.dataset.currentid||null;var s=a?stir.filter(t=>t.id===Number(a),t):null,s=s.length?s[0]:null,s=s?s.tags:null,s=stir.filter(C(s)),i=stir.filter(t=>t.id!==Number(a)),s=stir.compose(i,s,stir.sort(b),$)(t),i=stir.compose(i,stir.sort(k),stir.sort(b),$)(t),t=[...s,...i];stir.compose(I(d),w,g,p,r)(t)}}})}}(),function(t){t&&gallery&&galleryId&&stir.curry((t,r)=>(stir.setHTML(t,r),!0))(t,gallery.map(t=>`<img alt="Stirling AMAM Golf Day 2019" class="u-object-cover"  src="https://farm${t.farm}.staticflickr.com/${t.server}/${t.id}_${t.secret}_c.jpg" width="${t.o_width}" height="${t.o_height}"></img>`).join("")+`<div>
