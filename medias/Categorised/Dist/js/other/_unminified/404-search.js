...
(function () {
  ...
  var mediaIconHtml = '<span class="c-site-search__external-icon-wrapper"><span class="c-site-search__external-icon uos-document"></span> <span class="show-for-sr">Media</span></span>'; // object to hold the config data with default values
  ...
  var formResultHTML = function formResultHTML(myData) {
    ...
    rstHtml += '	<div class="grid-x">'; // Curator (BestBet)
    ...
    for (var key in rst) {
      ...
      if (val.displayUrl) {
        ...
        if (urlType === 'EXTERNAL') iconHtml = extIconHtml;
        if (urlType === 'MEDIA') iconHtml = mediaIconHtml;
        if (urlType === 'STAFF' || urlType === 'STUDENT') iconHtml = intIconHtml;
  ...
  var getSiteArea = function getSiteArea(myUrl) {
    ...
    if (arrUrl[3] === 'internal-students' || arrUrl[2] === 'portal.stir.ac.uk') return 'STUDENT';
    if (arrUrl[3] === 'internal-staff') return 'STAFF';
    if (arrUrl[3] === 'media') return 'MEDIA';
    return 'EXTERNAL';
  };
  /*
