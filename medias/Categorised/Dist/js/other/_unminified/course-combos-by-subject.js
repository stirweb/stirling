...
(function (scope) {
  ...
  var subjects = stir.compose(stir.sort(null), filterEmpties, removeDuplicates, flatten, //some courses have multiple subjects
  extractCourseSubjects, //so we'll need to flatten the array
  stir.clone)(initialCourses); // Courses data - remove any with no apply codes:

  var courses = stir.filter(hasCode, initialCourses); // Combos data - clean up:

  var combos = stir.filter(function (item) {
    return item.title;
  }, initialCombos); // Curry-in the combos now; we'll use this function later to
  // match the combos to (a filterd list of) courses:

  var expandCoursesWithCombos = getCoursesCombos(combos); // Curry-in the courses now, then later we'll selectivly pass subjects we want to filter on:
  ...
  })(Array.prototype.slice.call(resultsArea.querySelectorAll('h2'))); // Finally get the accordions working
