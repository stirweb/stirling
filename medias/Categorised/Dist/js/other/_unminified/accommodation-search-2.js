...
(function (scope) {
  ...
  var main = function main(_resultsArea, _params, _initialData) {
    ...
    var setDOMResults = setDOMContent(_resultsArea); // Run the data through the currys
  ...
  var filterData = stir.curry(function (_params, element) {
    ...
    var matcherCurry = stir.map(matcher(element)); // Run the data through the currys
  ...
  var setQueryParams = function setQueryParams(_params, _inputFilters) {
    ...
    }, _inputFilters); // Set new query params with form values
  ...
  if (!initialData.length) return; // Hide filter panels
