...
(function (scope) {
  ...
  */
  ...
  var processData = function processData(node_, resultNode, filterSelect, initialData_, subjectFilterValue) {
    ...
    var filteredDataSubject = stir.filter(filterData(filtersAllSubject), filteredData); // Subject select list

    var subjectFilters1 = filteredData.map(function (el) {
      return el.subject;
    }).join(", ").split(", ");
    var subjectFilters = stir.removeDuplicates(subjectFilters1).sort();
    setDOMContent(filterSelect, renderFilterOptions(subjectFilters, subjectFilterValue)); // How the data should be displayed

    var view = node_.dataset["render"] ? node_.dataset["render"] : ""; // Curry helper functions

    var sortDataCurry = stir.sort(function (a, b) {
      return a.title > b.title ? 1 : b.title > a.title ? -1 : 0;
    });
    var renderData = render(view);
    var setDOMResults = setDOMContent(resultNode); // Pass the raw data through the curry functions till it reaches the page
