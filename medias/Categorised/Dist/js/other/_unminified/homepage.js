(function () {
  ...
  if (elem = document.querySelector("[data-videoId]")) {
    elem.addEventListener("ended", function (e) {
      ...
      if (fallback = elem.getAttribute("data-fallback-html")) {//elem.style.display = 'none';
...
(function () {
  ...
  function doFeeds(container) {
    ...
    var processItems = function processItems(items, container, position) {
      ...
      if ("news" == container.getAttribute("data-promo-type")) scaffold.classList.add("large-6", "medium-12"); // primary items will be *pre*pended (so they appear first) and non-primary
  ...
  function testNewsEventItem(items, el) {
    ...
    if (typeof callback != "function") return console.error("callback is not a function"); // Check: items must be an array, and not empty
  ...
  function renderNewsEventItem(item, type) {
    ...
    var time = item._uos.startDate ? '<time class="u-block u-my-1 u-grey--dark>' + item._uos.startDate + endDate + "</time>" : ""; //var linktext = item.linktext || "Read more";
    ...
    isnews && html.push('<p class="text-sm">' + item.summary + "</p>"); //item.url     && html.push('<a href="' + item.url + '" class="c-link">' + linktext + '</a>');
