...
stir.snow = function () {
  ...
  function Snow(maxParticles) {
    ...
    W = 0;
    H = 0;
    draw = this.draw.bind(this); //canvas dimensions
    //snowflake particles
    ...
    this.ctx.strokeStyle = "white"; //angle will be an ongoing incremental flag.
    //Sin and Cos functions will be applied to it to
    //create vertical and horizontal movements of the flakes

    this.angle = 0.01; //begin animation loop
  ...
  Snow.prototype.update = function () {
    ...
    for (var i = 0; i < this.mp; i++) {
      var p = this.particles[i]; //Updating X and Y coordinates
      //We will add 1 to the cos function to prevent negative values which will lead flakes to move upwards
      //Every particle has its own density which can be used to make the downward movement different for each flake
      //Lets make it more random by adding in the radius

      p.y += p.rate;
      p.x += sin; //Sending flakes back from the top when it exits
