...
(function () {
  ...
  document.querySelectorAll('[data-funnelback-inject]').forEach(function (el) {
    ...
    if ("subject" === metaName) {
      ...
      metaValue = metaValue.split(', ').map(function (value) {
        return "\"".concat(value, "\"");
      }).join(" ");
    }
    var url = stir.funnelback.getJsonEndpoint().toString() + "?collection=".concat(collection, "&query=!padre&meta_").concat(metaName, "_orsand=").concat(metaValue).concat(parameters[type]);
