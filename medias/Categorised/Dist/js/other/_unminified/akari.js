...
stir.akari = function () {
  ...
  var tabulate = function tabulate(iterable) {
    var output = []; //console.info(typeof iterable, iterable)

    output.push("<table>");

    for (var element in iterable) {
      output.push("<tr>");
      output.push("<th>".concat(element).concat(Array.isArray(iterable[element]) ? '<br>[' + iterable[element].length + ' item(s)]' : '', "</th>")); //			if (Array.isArray(iterable[element])) {
      //				output.push(`<td>`);
