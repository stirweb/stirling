...
stir.akari = (function () {
  ...
  const render = (data) => {
    ...
    browser.innerHTML = (data.moduletitle ? `<h2 class=header-stripped>${data.moduletitle}</h2>` : "") + (data.modulecode ? `<p>${data.modulecode}</p>` : "") + ("object" === typeof data ? tabulate(data) : "") + ("string" === typeof data ? data : ""); //+ //'<p>Data for this module code is not found in Akari</p>'
