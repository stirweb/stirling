...
(function() {
     ...
     */
	var outputResults = function (resultSet){
		...
		if(lastPost > totalPosts)
			lastPost = totalPosts;
		
		resultSummaryArea.innerHTML = StirSearchHelpers.formSearchSummaryHTML(totalPosts, searchFacets.start_rank, lastPost);
		resultsArea.innerHTML = formResultHTML(resultSet) ; 
		
	    // Add pagination if required
	    paginationArea.innerHTML = '';
		if(postsPerPage < totalPosts)   
		    paginationArea.innerHTML = StirSearchHelpers.formPaginationHTML(totalPosts, postsPerPage, page, noOfPageLinks);
	...
	searchFormArea.addEventListener("submit", function(e){
		...
		QueryParams.set("year", searchFacets.meta_d); 
