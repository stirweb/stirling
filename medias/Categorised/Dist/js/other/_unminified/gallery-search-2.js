...
(function (scope) {
  ...
  */
  ...
  var main = function main(filters, consts, initData) {
    ...
    if (!gotData(initData)) return setDOMContent(consts.resultsArea, renderNoResults()); // Helper Curry function
    ...
    var setDOMResult = meta.page === "1" ? setDOMContent(consts.resultsArea) : appendDOMContent(consts.resultsArea); // Helper Curry functions
  ...
  */
  ...
  }, CONSTS.searchInputs); // Fetch for the data

  stir.getJSON(CONSTS.jsonUrl, function (initialData) {
    main(QueryParams.getAllArray(), CONSTS, initialData); // Listener: Form submit
    ...
    CONSTS.searchForm && CONSTS.searchForm.addEventListener("submit", function (e) {
      ...
      e.preventDefault();
      return;
    }, false); // Listener: Pagination clicks
