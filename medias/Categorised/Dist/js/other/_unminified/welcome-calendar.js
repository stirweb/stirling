(function () {
  ...
  var early = function early(event) {
    return startDate ? new Date(event.getAttribute('data-date')) > startDate : true;
  };

  var late = function late(event) {
    return endDate ? new Date(event.getAttribute('data-date')) < endDate : true;
  };

  var expired = function expired(event) {
    return new Date(event.getAttribute('data-date')) >= today;
  };
  ...
  var addback = function addback(event) {
  ...
  eventElements.sort(datesort).filter(early).filter(late).filter(expired).forEach(addback); // dates for the <select> should be unique

  dates = dates.filter(onlyUnique); // clean up some formatting:

  timeElements.forEach(function (element) {
    return element.innerText = element.innerText.replace(':00', '');
  });
  ...
  if (dates.length > 1) {
    ...
    for (var i = 0; i < dates.length; i++) {
      ...
      option.innerText = "".concat(("0" + date.getUTCDate()).slice(-2), " ").concat(months[date.getUTCMonth()], " ").concat(date.getFullYear());
      dateSelectElement.appendChild(option);
    }
  ...
  function hideDeselectedElements(selector) {
    var elements = selector ? Array.prototype.slice.call(calendar.querySelectorAll(".h-event:not(".concat(selector, ")"))) : [];
  ...
  function getDateSelector() {
    ...
    return date && date.value ? "[data-date=\"".concat(date.value, "\"]") : '';
  }

  function getGroupSelector() {
    var group = groupSelectElement.options[groupSelectElement.selectedIndex];
    return group && group.value ? "[data-group*=\"".concat(group.value, "\"]") : '';
  }
