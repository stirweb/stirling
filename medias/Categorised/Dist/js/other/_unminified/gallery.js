...
(function () {
  ...
  var getElement = function getElement(tagName) {
    return tagName && document.querySelector(tagName);
  };
  ...
  var firstText = function firstText(text) {
    return text && text.split("|").shift().trim();
  };

  var genericTitle = firstText(getText(getElement('title'))) || firstText(getText(getElement('h1'))); // Get the photoswipe HTML template and insert it into the DOM:
  ...
  for (x = 0; x < window[galleryID].length; x++) {
    ...
    image.setAttribute('alt', photo.title || genericTitle || "Image ".concat(x, " of ").concat(window[galleryID].length));
    image.setAttribute('data-gallery-index', x); //link.appendChild(image);
  ...
  if (typeof PhotoSwipe !== "undefined") {
    galleryEl.addEventListener("click", function (event) {
      ...
      new PhotoSwipe(document.querySelectorAll('.pswp')[0], PhotoSwipeUI_Default, Array.prototype.slice.call(items), {
        index: parseInt(event.target.getAttribute('data-gallery-index'))
      }).init();
    }, true);
