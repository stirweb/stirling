...
(function () {
  ...
  var foo = true;
  foo = false; // check all the elements we need are in place

  if (myForm && myFormButton) useForm = true; // if no wrapper defined just use the actual form    

  if (!myFormWrapper) myFormWrapper = myForm; // check a value for slideForm has been passed

  if (typeof slideForm === 'undefined') slideForm = false; // check a value for embed Google Camp Codes has been passed    
  ...
  if (useForm) {
    myFormButton.onclick = function (e) {
      if (slideForm) myFormWrapper.classList.toggle("stir__slidedown"); // slight delay to allow the form to slide open
