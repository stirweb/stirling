 ...
 */    
 ...
 (function() {
    ...
    if(!myTarget && myTarget2)
        myTarget = myTarget2;
    
    // check all the elements we need are in place
    if(myTarget && myScrollButton)
        useScroll = true;

    // if no wrapper defined just use the actual target    
    if(!myTargetWrapper)
        myTargetWrapper = myTarget;
    
    // check a value for embed Google Camp Codes has been passed    
    if(typeof embedGCC === 'undefined')
        embedGCC = true;  
    ...
    if(useScroll && embedGCC){ 
        ...
        if(document.getElementById('UTM_CAM'))
            document.getElementById('UTM_CAM').value = ls_utm_campaign;
        
        if(document.getElementById('UTM_MEDIUM'))   
            document.getElementById('UTM_MEDIUM').value = ls_utm_medium;
            
        if(document.getElementById('UTM_SOURCE')) 
            document.getElementById('UTM_SOURCE').value = ls_utm_source;
    }
    ...
    if(useScroll){ 
        myScrollButton.onclick = function(e){ 
            ...
            // animate slide in if enabled
            if(aniSlideInTarget)
                myTargetWrapper.classList.toggle("stir__slidedown");
            
            // slight delay to allow the target element to slide open
            setTimeout(function() {
