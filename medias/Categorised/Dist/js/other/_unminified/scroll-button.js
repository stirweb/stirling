...
(function () {
  ...
  if (!myTarget && myTarget2) myTarget = myTarget2;

  // check all the elements we need are in place
  if (myTarget && myScrollButton) useScroll = true;

  // if no wrapper defined just use the actual target    
  if (!myTargetWrapper) myTargetWrapper = myTarget;
  ...
  if (useScroll) {
    myScrollButton.onclick = function (e) {
      ...
      if (aniSlideInTarget) myTargetWrapper.classList.toggle("stir__slidedown");
