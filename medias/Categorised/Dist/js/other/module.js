!function(){function c(){const t=new IntersectionObserver(function(e,t){e.forEach(e=>{var t,r,a,i;e.isIntersecting?(t=Number(e.target.dataset.value),r=e.target.dataset.unit,a=Number(e.target.dataset.max),i=e.target.dataset.colour||"energy-turq",a=t/a*100,i=stir.createDOMFragment(`<div class="barchart-value u-bg-${i} u-absolute" style="right:${100-a}%"></div>
                                                <div class="barchart-text u-relative u-white u-font-bold text-md u-z-50" style="left:${a/2-2}%">${t}${r}</div>`),e.target.append(i)):e.target.innerHTML=""})},{root:null,threshold:.5});stir.nodes(".barchart").forEach(e=>{t.observe(e)})}const g=(e,t)=>{return`<div class="cell medium-9 bg-grey u-bleed u-p-2 ">
              ${t.disclaimer||""}
          </div>
          ...
          `},h=({moduleTitle:e,moduleCode:t,moduleLevel:r,moduleCredits:a})=>`<div class="grid-container">
            ...
            </div>`,m=({moduleCredits:e,ectsModuleCredits:t,colourPack:r},a)=>`<div class="cell u-mt-2">
        ...
        </div>`)),a=stir.curry((e,t,{tab:r,tabAssessments:a})=>{e=i(e);return(1<t?`<h4 class="u-mt-0">${r}</h4>`:"")+`<p>${a.map(e).join("")}</p>`}),s=stir.curry((e,t)=>t?`<div class="cell large-${e} u-mb-1">${t}</div>`:""),b=(e,t,r,a)=>{var i=e.length?(i="12",(e=e)?`<div class="cell large-${i} u-mb-1">${e}</div>`:""):a.deliveriesFallback,e=(console.log(t.length),t.length<2?"12":"6"),e=s(e),e=t.length?t.map(e).join(""):'<div class="cell">'+a.assessmentFallback+"</div>";return`<div class="cell">
        ...
        </div>`},f=()=>'<div class="grid-container"><div class="grid-x grid-padding-x">',$=()=>"</div></div>",x=()=>f()+'<div class="cell u-padding-y"><h1>Page not found</h1></div>'+$(),w=stir.curry((e,t)=>(stir.setHTML(e,t),!0)),y=e=>e&&e.toLowerCase().includes("p")?"pg":"ug",k=(t,e)=>(e.filter(e=>e.level===t).length?e.filter(e=>e.level===t):e)[0];const d=e=>{return{sum:e.tabAssessments.map(e=>Number(e.percent)).reduce((e,t)=>e+t,0),assessment:e}},C=(e,t)=>{e=e.map(d);const r=a(t,e.length);return e.map(e=>100!==e.sum?"":r(e.assessment))};var e=stir.moduleTexts||{},t=new URLSearchParams(document.location.search);!async function(t,e,r){var a,i,s,d,l,n,u=await fetch(t);try{var o=await u.json();a=o,i=e,s=r,(n=stir.node("#content")).classList.add("u-padding-bottom"),a.error?w(n,x()):(l=y(a.moduleLevelDescription),i=k(l,i),d={...a,colourPack:i,boilerplates:s},a=a.assessments||[],a=C(a,i),l=h(d)+f()+g(l,s)+p(d)+b("",a,i,s)+m(d,s)+v(d,s)+$(),w(n,l)),c()}catch(e){w(stir.node("#content"),x()),console.log(e.message),console.log(t)}}("https://www.stir.ac.uk/data/courses/akari/module/index.php?module="+[t.get("code"),t.get("session"),t.get("semester")].join("/"),[{level:"ug",first:"heritage-green",second:"energy-turq",third:"energy-purple"},{level:"pg",first:"heritage-purple",second:"heritage-purple",third:"heritage-green"}],e)}();