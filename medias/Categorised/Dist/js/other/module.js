!function(){function n(){const t=new IntersectionObserver(function(e,t){e.forEach(e=>{var t,a,r,s;e.isIntersecting?(t=Number(e.target.dataset.value),a=e.target.dataset.unit,r=Number(e.target.dataset.max),s=e.target.dataset.colour||"energy-turq",r=t/r*100,s=stir.createDOMFragment(`<div class="barchart-value u-bg-${s} u-absolute" style="right:${100-r}%"></div>
                                                <div class="barchart-text u-relative u-white u-font-bold text-md u-z-50" style="left:${r/2-2}%">${t}${a}</div>`),e.target.append(s)):e.target.innerHTML=""})},{root:null,threshold:.5});stir.nodes(".barchart").forEach(e=>{t.observe(e)})}const l=(e,t,a)=>{var r=new URLSearchParams(document.location.search);return`<div class="cell medium-9 bg-grey u-bleed u-p-2 ">
                <p>The module information below is for the ${r.get("session")?r.get("session"):"current year"} intake and may be subject to change, including in response to student feedback and continuous innovation development. See our <a href="${t}">terms and conditions</a> for more information.</p>
            </div>
            <div class="cell medium-3 align-middle align-center u-flex">
            ${r=e,t=new URLSearchParams(document.location.search),t.get("course")?`<a href="${stir.isNumeric(t.get("course"))?"/terminalfour/preview/1/en/"+t.get("course"):`/courses/${r.replace("pg","pg-taught")}/`+t.get("course")}#panel_1_3" id="backtocourseBtn" class="button u-m-0 heritage-green button--back ">Back to course</a>`:""}
