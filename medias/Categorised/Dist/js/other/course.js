...
var StirUniModules = function () {
  ...
  var ver, debug;
  var _options = {}; // need to remember these for callbacks
  ...
  var _semester = {
    getYear: function getYear(semesterCode) {
      return Math.ceil(semesterCode / _semestersPerYear);
    },
    getSemesterYearIndex: function getSemesterYearIndex(semesterCode) {
      var mod = semesterCode % _semestersPerYear;
      return mod === 0 ? _semestersPerYear : mod;
    }
  };
  var _option = {
    getYearFromMonth: function getYearFromMonth(month) {
      if (!month) return;
      ...
      switch (month) {
        ...
        case "August":
          return '2022';
        //new Date().getFullYear() + 1; // return next year
  ...
  var _loading = {
    el: _createLoaderHTML(),
    // HTML element to show loading activity
  ...
  var _insertCache = function _insertCache(id, data) {
    _cache[id] = JSON.parse(JSON.stringify(data));
  };

  var _getCached = function _getCached(id) {
    return _cache[id];
  };

  function _createLoaderHTML() {
    var element = document.createElement("div");
    var innerHTML = ["\t" + '<div class="loader"></div>', "\t" + '<span class="show-for-sr">Loading modules...</span>'];
  ...
  function _getVersions(callback) {
    $.ajax({
      ...
      dataType: "script",
      cache: true,
      success: callback || new Function() //,error: function() {}
  ...
  function _loadCourseRoutes(routeCodesCSV, courseType, options) {
    ...
    _courseType = courseType; // get, and set _options

    options = _getOptions(options); // if there is only one route, just jump into it (no need to present one option to user)
    ...
    if (_routeCodes.length === 1 && _routeCodes[0]) {
      ...
      $.ajax({
        ...
        dataType: "script",
        cache: true,
        success: function success(data) {//uses JSONp callback insted; StirUniModules.showCourseRoutes()
        },
        error: function error() {
          _hideLoading("_loadCourseRoutes");

          _showCourseRoutesError({
            routeCodes: _routeCodes
          });
        }
      });
    }
  ...
  function _reset(all) {
    ...
    while (_DOM['accordion'].firstChild) {
      _DOM['accordion'].removeChild(_DOM['accordion'].firstChild);
    }
  }
  /**
  ...
  function _showCourseRoutes(data) {
    ...
    var options = _getOptions(),
        routesData = [],
        // will contain the subset of routes we need
    pointer = 0;
  ...
  function _loadCourseOptions(roucode, courseType, options) {
    ...
    _setSemestersPerYear(); // get, and set _options
  ...
  function __courseOptionFilter(occurrence) {
    var occ = occurrence[2],
        season = occurrence[3]; // if the code begins with "F" then the season ought NOT to be Autumn: 

    if (occ.charAt(0) === 'F' && season === "Autumn") return false; // otherwise consider it valid (i.e. return true)

    return true; // TODO? Some other mappings we might want to test for:
  ...
  function _showCourseOptions(data) {
    // get, and set _options
    options = _getOptions(); // validate the data before caching and/or using it

    data = _validateCourseOptions(data); // cache data

    _insertCache(_url, data); //if there is only one route, just jump into it (no need to present one option to user)
  ...
  function _loadModule(moduleCode, semesterCode, occurrence, yearSession) {
    ...
    $.ajax({
      ...
      dataType: "script",
      cache: true,
      success: function success(data) {// uses JSONp callback; StirUniModules.showModule()
      },
      error: function error() {
        _showModuleError({
          "details": [moduleCode, semesterCode, occurrence, yearSession]
        });
      }
    });
  }
  ...
  var _showCourseRoutesRenderer = function _showCourseRoutesRenderer(routesData) {
    // get, and set _options
    options = _getOptions(); //var $optslist = $(options.container);
    ...
    html.push('</select>'); //$optslist.prepend( html.join("") );

    options.container.insertAdjacentHTML("afterbegin", html.join(""));
  };

  var _showCourseRoutesErrorRenderer = function _showCourseRoutesErrorRenderer(data) {
    console.error("There was a problem loading course routes data", data);
  };

  var _showCourseOptionsRenderer = function _showCourseOptionsRenderer(data) {
    options = _getOptions(); //var $optslist = $(_getOptions()["container"]);
    ...
    html.push('</ul>'); //$optslist.html( html.join("") );

    options.container && (options.container.innerHTML = html.join(""));
  };

  var _showCourseOptionsErrorRenderer = function _showCourseOptionsErrorRenderer(data) {
    console.error("There was a problem loading course options data", data);
  };

  var _showCourseModulesRenderer = function _showCourseModulesRenderer(semestersData) {
    console.error("showCourseModulesRenderer() is not defined");
  };

  var _showCourseModulesErrorRenderer = function _showCourseModulesErrorRenderer(data) {
    console.error("There was a problem loading modules data", data);
  };

  var _showModuleRenderer = function _showModuleRenderer(data) {
    console.error("showModuleRenderer() is not defined");
  };

  var _showModuleErrorRenderer = function _showModuleErrorRenderer(data) {
    console.error("showModuleErrorRenderer() is not defined");
  };

  var _getSemestersPerYear = function _getSemestersPerYear() {
    return _semestersPerYear;
  };

  var _getSelectedRoute = function _getSelectedRoute() {
    return _selectedRoute;
  };

  var _getCourseType = function _getCourseType() {
    return _courseType;
  };
  ...
  function _getOptions(options) {
    ...
    if (!options) options = {}; // set defaults, get _options, and apply any new options

    _options = stir.Object.extend({
      //$.extend(true, {
  ...
  var _showLoadingRenderer = function _showLoadingRenderer() {
    console.error("Show loading renderer not defined");
  };

  var _hideLoadingRenderer = function _hideLoadingRenderer() {
...
stir.components.discoveruni.widget = function (options) {
  var widget = document.createElement("div"); //add this class if we want DiscoverUniWidget to trigger automatically:
    ...
    (function (d) {
      ...
      widgetScript.addEventListener("load", function (event) {
        if (widgets.length > 1 && window.DiscoverUniWidget) {
          ...
          contentInsertionNode = contentInsertionNode.querySelector("[data-tab-content]"); // patch DiscoverUniWidget's addCss() function so it only runs once per page (not once per widget!)
          ...
          DiscoverUniWidget.prototype.renderWidget = function () {
            ...
            this._renderWidget.apply(this, arguments); // if the widget has no data we'll do nothing further


            if (this.targetDiv.classList.contains("no-data")) return; // skip the first widget but put the rest into a <details> accordion
...
var KISWidgetCaller = function KISWidgetCaller() {
  return false;
};
/*
 * Clearing
...
(function (scope) {
  ...
  function toggle(flag) {
    if (this.nodeType === 1) flag ? this.classList.remove('hide') : this.classList.add('hide');
  }
  ...
  function handleChanges() {
    ...
    hideAll(); // Then, only reveal the ones that match the selected region.
  ...
  hideAll(); // Now listen for the user:

  if (!select.id) select.id = 'change-region';
  select.addEventListener('change', handleChanges); // Set up any remote controls. Each `remote` should just be
...
StirUniModules.setShowRoutesRenderer(function (routesData) {
  ...
  selector.addEventListener("change", function (e) {
    ...
    var courseType = value[1]; // Reset various DOM elements
...
StirUniModules.setShowOptionsRenderer(function (optionsData) {
  ...
  for (var i = 0; i < optionsData.length; i++) {
    ...
    var innerText = "Starting " + optionsData[i][3] + // " " +
  ...
  optList.appendChild(selector); // set behaviour of each select
  ...
  selector.addEventListener("change", function (e) {
    ...
    var moa = value[1];
    var occ = value[2];
    StirUniModules.reset(false); // this will load and display the modules:
...
StirUniModules.setShowModulesRenderer(function (data) {
  ...
  var module,
      ...
      semesterCache = [],
      semesterNameText,
      //e.g. "Semester 1" / "Speing semester"
  semesterOptionIdText,
      //if multiple options exist (e.g. " (Option A)")
  showCollectionNotes,
      // boolean
  collectionId,
      //string
  ...
  var getCollectionHeaderText = function getCollectionHeaderText(collectionStatusCode) {
    var collectionHeaderText; // this was taking from how the calendar JS displays titles
  ...
  var getCollectionHeaderChecksum = function getCollectionHeaderChecksum(code, notes) {
  ...
  for (var g = 0; g < data.semesterGroupBeans.length; g++) {
    ...
    for (var o = 0; o < data.semesterGroupBeans[g].groupOptions.length; o++) {
      ...
      for (var s = 0; s < data.semesterGroupBeans[g].groupOptions[o].semestersInOption.length; s++) {
        ...
        semesterId = [g, o, s].join(""); // first, we'll work out which academic year we are in
        ...
        collections.sort(StirUniModules.moduleSort); // now we have all the info we need to render the HTML:

        semesterNameText = "Year " + year + ", " + (semester.semesterName ? semester.semesterName + " semester" : "Semester " + StirUniModules.semester.getSemesterYearIndex(semester.semesterCode)); //multi-option year
        ...
        html.push('<div class="c-wysiwyg-content c-course-modules__collection ' + StirUniModules.css.truncateModuleCollection + '" id="panel_modules_' + semesterId + '" role="region" aria-labelledby="accord_modules_' + semesterId + '">'); // reset this to ensure on the next semester block title is output

        collectionHeaderChecksum = null; // insert accordion content
        ...
        for (var c = 0; c < collections.length; c++) {
          ...
          for (var m = 0; m < collection.mods.length; m++) {
            module = collection.mods[m]; // hide the module if it's unavailable. (This condition was taken from calendar js).
          ...
          if (collectionNotEmpty || debug) {
            ...
            showCollectionNotes = collection.collectionType == "LIST" || collection.collectionType == "CHOICE"; // we'll show the header if there are collection notes, if header text has changed,
            ...
            collectionHeaderChecksum = getCollectionHeaderChecksum(collection.collectionStatusCode, collection.collectionNotes); // now output the table we created earlier
  ...
  function resetModuleModal() {
    ...
    body && (body.innerHTML = ""); // show loading, hide content
    //$(loading).show();
    //$(content).hide();
...
StirUniModules.setShowRoutesErrorRenderer(function (data) {
  ...
  if (data.routeCodes && data.routeCodes.length) {
    data.routeCodes.forEach(function (route) {
      ...
      container.insertAdjacentHTML("beforeend", html); // $("#course-modules-container").append(
      //   '<p><a href="' +
...
StirUniModules.initialisationRoutine = function () {
  ...
  return function (event) {
    ...
    StirUniModules.loadCourseRoutes(rouCode, courseType); // flag to suppress loading modules again
...
(function () {
  var elem = stir.node("#course-modules-container");
...
(function (scope) {
  ...
  var addCourseItemListener = function addCourseItemListener(el) {
    if (el.children[0] && el.children[0].hasAttribute("href")) el.onclick = function () {
      return window.location = el.children[0].attributes.href.value;
    };
  };
  ...
  stir.each(function (el) {
    return addCourseItemListener(el);
  }, stir.nodes(".c-course-related__buttons ul li"));
  /* End */
...
(function () {
  /*
    DOM elements
  */
  var applySlideBtns = stir.nodes("[data-applyslidelink]");
  var skipLinks = stir.nodes(".skip-link");
  var buttonsNode = stir.node(".c-course-title__buttons");
  /*
    Vars
  */
  ...
  var errorMsg = ["WARNING!! Missing apply now button anchor location ID", "Please inform a dev!! "];
  if (!applySlideBtns && !skipLinks) return;
  /* 
    Returns a node that outputs an error message
  */

  var renderErrorNode = function renderErrorNode(contentId, errorMsg) {
    return "<p class=\"alert callout\">" + errorMsg[0] + " " + contentId + " " + errorMsg[1] + "</p>";
  };
  /* 
  ...
  */
  ...
  var getLinkId = function getLinkId(href) {
    var contentUris = href.split("#");
  ...
  */
  ...
  var isInComponent = function isInComponent(className, node) {
    var found = false,
        tempNode = node;
  ...
  var slideToContent = function slideToContent(nodeId, offset) {
    var node = document.getElementById(nodeId);
    var tab = isInComponent("stir-tabs__content", node); // Open the tab if found and closed

    if (tab) {
      var tabBtn = tab.previousElementSibling;
      if (!tabBtn.classList.contains("stir-tabs__tab--active")) tabBtn.click();
    }

    var accord = isInComponent("stir-accordion", node); // Open the accord if found and closed

    if (accord) {
      var accordBtn = accord.children[0].children[0];
  ...
  var handleClick = function handleClick(e) {
    var contentId = getLinkId(e.target.href);

    if (contentId && document.getElementById(contentId)) {
      var offset = offsets[stir.MediaQuery.current] ? offsets[stir.MediaQuery.current] : 100;
  ...
  var addClickListener = function addClickListener(btn) {
  ...
  */
  ...
  var handleError = function handleError(btn, buttonsNode, errorMsg) {
    var contentId = getLinkId(btn.getAttribute("href"));
...
(function (scope) {
  ...
  var DATANODE = scope; //const server = 'stir-search.clients.uk.funnelback.com';

  var server = "search.stir.ac.uk";
  var urlBase = "https://".concat(server); // ClickTracking

  var JSON_BASE = "".concat(urlBase, "/s/search.json?");
  var scaleImage = stir.curry(function (server, image) {
    return "https://".concat(server, "/s/scale?url=").concat(encodeURIComponent(image), "&width=800&height=800&format=jpeg&type=crop_center");
  });
  ...
  var formatName = function formatName(name) {
    return name.replace("| Student Stories | University of Stirling", "").trim();
  };
  /*
  ...
  var renderQuote = function renderQuote(item) {
    return "\n      <div id=\"pullquote-77513\" class=\"pullquote pullquote-pic \">\n            <img src=\"".concat(scaleImageWithFunnelback("https://stir.ac.uk" + item.metaData.profileImage), "\" alt=\"").concat(formatName(item.title), "\" loading=\"lazy\"  width=\"700\" height=\"600\">\n            <div class=\"pullquote--text\">\n              <cite>\n                <span class=\"author\">").concat(formatName(item.title), "</span><br>\n                <span class=\"info\">").concat(item.metaData.profileCountry, "</span><br>\n                <span class=\"info\">").concat(item.metaData.profileDegreeTitle, "</span>\n              </cite>\n              <blockquote cite=\"#\">\n                <q>").concat(item.metaData.profileSnippet, "</q>\n              </blockquote>\n              <a href=\"").concat(item.displayUrl, "\" aria-label=\"Read ").concat(formatName(item.title), "'s story (").concat(formatName(item.title), ")\" class=\"c-link\">Read ").concat(formatName(item.title).split(" ")[0], "'s story</a>\n            </div>\n        </div>\n    ");
  };
  /*
  ...
  var setDOMContent = function setDOMContent(resultsArea, html) {
  ...
  var getTestimonial = function getTestimonial(testimonial, results) {
  ...
  var main = function main(dataNode, consts, jsonBase) {
    var testimonials = dataNode.getAttribute("data-testimonials").split("|").filter(function (el) {
      return el;
    });
    if (!testimonials.length) return false;
    var params = Object.entries(consts).map(function (item) {
      return item[0] + "=" + item[1];
    }).join("&");
    var query = "&query=[".concat(testimonials.map(function (el) {
      return el;
    }).join(" "), "]");
    var jsonUrl = jsonBase + params + query; // Get the data from FunnelBack

    stir.getJSON(jsonUrl, function (initialData) {
      var results = initialData.response.resultPacket.results;
      var html = testimonials.map(function (item) {
        return getTestimonial(item, results);
      }).join(" ");
      var resultsArea = dataNode.querySelector(".pullquotesbox");
  ...
  var run = DATANODE.map(function (element) {
    return main(element, CONSTS, JSON_BASE);
  });
  /*
      End
   */
