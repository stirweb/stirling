...
stir.templates.course = {
	...
	disclaimer: `<p><strong>The module information below provides an example of the types of course module you may study. The details listed are for the academic year that starts in September 2026. Modules and start dates are regularly reviewed and may be subject to change in future years.</strong></p>`
};
...
stir.course = (function () {
	...
	const source = container.hasAttribute("data-modules-source") ? container.getAttribute("data-modules-source") : 'api';
	const useAPI = "api"===source;
	
	debug && console.info('[Course] DOM checks passed. Data source: ', source);
	...
	debug && console.info('[Course] parameters',parameter);
	...
	*/
	...
	const auto = useAPI ? _auto : override;
	
	// Consider the CALLBACK QUEUE:
	if (stir.callback && stir.callback.queue && stir.callback.queue.indexOf("stir.course.auto") > -1) auto();
	// todo: empty the queue?

	return {
		init: data => boilerplates = data, // get module boilerplate text
		auto: auto, // initialise and begin
	};
})();
