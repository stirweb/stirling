...
stir.templates.search = (() => {
  ...
  const authMessage = (group) =>
    notice(
      `This page is only available to ${groups[group] || group}. You will be asked to log in before you can view it, but once you are logged in results will be shown automatically.`,
    );
  ...
  const makeBreadcrumbs = (item) => {
    const crumbs = {
      text: item.custom_fields.breadcrumb
        ? item.custom_fields.breadcrumb.split(" > ").slice(1, -1)
        : item.categories.slice
          ? item.categories.slice(1, -1).map((crumb) => crumb.split("x")[1])
          : [""],
  ...
  const searchParamTokens = (parameters) =>
    Array.from(parameters.entries())
      .map((item) => (item[1] && item[1].indexOf('"') >= 0 ? "" : paramToken(item[0], item[1]) || ""))
      .join(" ");
  ...
  const getTags = (tags) => {
    const html = Object.values(tags || {})
      .map((i) => i.map((j) => (j.desc ? stir.templates.search.stag(j.desc) : "")).join(" "))
      .join(" ")
      .trim();
  ...
  return {
    ...
    auto: (item, index, context) => {
      ...
      if (item.custom_fields.type) {
        if (String === item.custom_fields.type.constructor) {
          switch (item.custom_fields.type) {
            ...
            case "event":
              return stir.templates.search.event(item);
            case "webinar":
              return stir.templates.search.event(item);
            case "contract":
            case "area":
            case "centre":
              return stir.templates.search.research(item);
            case "profile":
              return stir.templates.search.person(item);
            case "studentstory":
              return stir.templates.search.studentstory(item);
            case "scholarship":
              return stir.templates.search.scholarship(item);
      ...
      const attrs = [`data-rank=${item.score || ""}`, `data-as-type="${item.type || ""}"`, `data-result-type="${item.custom_fields.type || ""}"`];
    ...
    facts: (item) => {
      ...
      if (item.custom_fields.start) {
        if (item.custom_fields.start.map) {
          facthtml.push(
            stir.templates.search.courseFact(
              "Start dates",
              item.custom_fields.start
                .map(startDateFormatter)
                .filter((x) => x)
                .join(", "),
              false,
            ),
    ...
    courseFact: (head, body, sentenceCase) =>
      head && body ? `<div class="cell medium-4"><strong class="u-heritage-green">${head}</strong><p${sentenceCase ? " class=u-text-sentence-case" : ""}>${body}</p></div>` : "", //.replace(/\|/g, ", ")
    ...
    event: (item) => {
      ...
      const renderCTA = () => {
        if (data.register && hasEnded) return `<p class="u-m-0 text-sm"><a href="${data.register}" class="u-m-0 button hollow tiny">Watch now</a></p>`;
        if (data.register && !hasEnded) return `<p class="u-m-0 text-sm"><a href="${data.register}" class="u-m-0 button hollow tiny">Register now</a></p>`;
        return ``;
      };
				...
				<div class="c-search-result__body flex-container flex-dir-column u-gap u-mt-1">
					...
					<div class="flex-container flex-dir-column u-gap-8">
						<div class="flex-container u-gap-16 align-middle">
							...
							<span>${hasEnded ? `Originally ran: ${datespan(item.custom_fields.d, item.custom_fields.e)}` : datespan(item.custom_fields.d, item.custom_fields.e)}</span>
						</div>
						<div class="flex-container u-gap-16 align-middle">
							<span class="uos-clock u-icon h5"></span>
							<span>
							${hasEnded ? "Available to watch on-demand" : timespan(item.custom_fields.d, item.custom_fields.e)}
							</span>
						</div>
						<div class="flex-container u-gap-16 align-middle">
...
stir.search = (() => {
  ...
  const constants = {
    ...
    parameters: {
      ...
      internal: {
        ...
        resultType: "organic",
        filter: JSON.stringify({
          or: [{ "custom_fields.access": "staff" }, { "custom_fields.access": "student" }],
        }),
      },
      clearing: {
        collectAnalytics: false,
  ...
  const addFilterParameters = (url, formData) => {
    ...
    for (let [key, value] of b) {
      "sort" === key ? a.set(key, value) : a.append(key, value);
    }
    url.search = a;
    return url;
  };
  ...
  const setQuery = () => {
    ...
    constants.form.term.value ? QueryParams.set("term", constants.form.term.value) : QueryParams.remove("term");
  };
  ...
  const renderPagination = stir.curry((type, data) => {
  ...
  const callSearchApi = stir.curry((type, callback) => {
    ...
    const parameters = stir.Object.extend(
      {},
  ...
  const clickReporter = async (event) => {
    ...
    const el = event.target.hasAttribute("data-docid")
      ? event.target
      : event.target.parentElement && event.target.parentElement.hasAttribute("data-docid")
  ...
  const searchReporter = async (query, total) => {
    ...
    stir.addSearch.putReport(payload).catch((error) => console.error(error));
  };
